...
function Calendar({ className, classNames, showOutsideDays = true, components: customComponents, ...props }: CalendarProps) {
  ...
  const _dropdownsClassName = cn("flex items-center justify-center gap-2 w-full", props.dropdownsClassName);
        ...
        >
          ...
          <SelectContent position="popper" align="center">
            <ScrollArea className="h-80">
              {options?.map(({ value: optionValue, label, disabled }, id) => (
                <SelectItem key={`${optionValue}-${id}`} value={optionValue?.toString()} disabled={disabled}>
                  {label}
                </SelectItem>
              ))}
            </ScrollArea>
